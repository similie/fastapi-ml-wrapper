...
def load_data_csv(data_dir: str,
    ...
    df = pd.concat(
        [
            ...
            for chunk in tqdm(
                pd.read_csv(
                    data_dir,
                    usecols=["station", "date", "precipitation"],
                    dtype=type_dict,
    ...
    precip = transforms[1].fit_transform(df.precipitation.values.reshape(-1,1))
    df.precipitation = precip
    df = generate_time_lags(df, "precipitation", 48)
    features = df.columns.to_list()
    features = [ x for x in features if x != 'station']
    if pred:        
        df = df[(df.index >= (daydelta(df.index.max(), -568))) & (df.index <= (daydelta(df.index.max(), -390)))] #lkg
    station = df.station
    df = pd.DataFrame(transforms[0].fit_transform(df[features].values),
                      columns=features,
                      index=df.index)
    df['station'] = station
    return df, transforms, features
