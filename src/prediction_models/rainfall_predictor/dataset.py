...
# load config and module level variables
...
def gen_sequence(df, sequence_length, seq_cols):
    ...
    for start, stop in zip(
        range(0, num_elements-sequence_length),
...
def gen_pred_dataset(df, sequence_length: int):
    '''
    Generate the dataset for predictions with historical and forecast data in
    the correct sequence length, ie: (batches, sequence_length, n_features)
    '''
    X_pred, y_pred = [], []

    # [sg] removed unsed iterator: for station, _df in df.groupby(["station"]):
...
def groupdf(df):
    '''
    TODO: Remove, only needed for training.
    '''
...
def onehot_transform(R_: np.array):
    '''
    Replaces the rainy_season column with a two-column one hot encoded feature.
    Rainy season must be the last feature.
    '''
    R = R_[:, :, -1:].astype('int')
