...
def plot_predictions(preds: any, og_data: pd.DataFrame, target: str = "precipitation"):
    """
    ...
    sns.set_style('darkgrid')
    sns.set(rc={'figure.figsize':(14,8)})
    preds.rename(columns={'precipitation': 'prediction'}, inplace=True)
    preds['prediction'].plot()
    og_data['precipitation'].plot()
